...
%Initialize Cells in Neural Tube
currcell = max(grid(:)) + 1;
for j = 3:15
    for k = 1:13
       dim1=0;
       if(mod(k,2)==0)
        ...
        grid(end-12*(j-1)+1+dim1:end-12*(j-2)+dim1,111+12*k:111+12*(k+1) - 1) = currcell;
        currcell = currcell + 1;
    end
end
...
if (videoWrite)
    v = VideoWriter('C:\Users\MH\Desktop\CellularPottsModel\CPM_tailbud.avi','Motion JPEG AVI');
    open(v);
end
...
for j = 1:3000
    ...
    if ( mod(j, 50) == 0 )
        ...
        for k = 1:14
            ...
            grid(end-18+dim1:end-6+dim1,111+12*(k-1)+6:111+12*k - 1+6) = currcell;
            currcell = currcell + 1;
        end
